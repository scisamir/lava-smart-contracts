...
import { PoolInfo, TOKEN_PAIRS, UserOrderType } from "@/lib/types";
import { setupE2e } from "./setup";
import { MintingHash } from "./mint/validator";
import { PoolValidatorAddr } from "./pool/validator";
  ...
  orderUtxos.filter((utxo) => {
    ...
    for (let i = 0; i < utxoAmount.length; i++) {
      ...
      if (
        ...
        utxoAsset.unit ===
        MintingHash + stringToHex(TOKEN_PAIRS[0].derivative)
      ) {
        ...
        utxoAsset.unit ===
        MintingHash + stringToHex(TOKEN_PAIRS[1].derivative)
      ) {
        ...
        utxoAsset.unit ===
        MintingHash + stringToHex(TOKEN_PAIRS[2].derivative)
      ) {
...
const getTotalOrderNumbers = async (
  blockchainProvider: BlockchainProviderType
) => {
  const orderUtxos = await blockchainProvider.fetchAddressUTxOs(
    OrderValidatorAddr
...
const fetchPoolInfo = async (blockchainProvider: BlockchainProviderType) => {
...
export { fetchUserOrders, getTotalOrderNumbers, fetchPoolInfo };
