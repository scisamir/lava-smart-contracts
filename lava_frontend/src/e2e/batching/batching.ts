...
export const batchingTx = async (
  ...
  const filteredOrderUtxos = orderUtxos.filter((utxo) => {
    ...
    for (let i = 0; i < utxoAmount.length; i++) {
      ...
      if (
        utxoAsset.unit === orderOptInUnit ||
        utxoAsset.unit === orderOptOutUnit
      ) {
        isRightOrder = true;
  ...
  const poolAssetAmount = poolUtxo.output.amount.find(
    (amt) => amt.unit === orderOptInUnit
  )?.quantity;
  const updatedPoolAssetAmount =
    Number(poolAssetAmount ?? "0") + totalMintAmount;

  const poolPlutusData = poolUtxo.output.plutusData;
  if (!poolPlutusData) throw new Error("No plutus data");
  const poolDatum = deserializeDatum<PoolDatumType>(poolPlutusData);
  const updatedTotalStMinted =
    Number(poolDatum.fields[1].int) + totalMintAmount;
  const updatedTotalUnderlying =
    Number(poolDatum.fields[2].int) + totalMintAmount;
  ...
  for (let i = 0; i < batchingOrderUtxos.length; i++) {
    ...
    const orderLovelace = orderUtxo.output.amount.find(
